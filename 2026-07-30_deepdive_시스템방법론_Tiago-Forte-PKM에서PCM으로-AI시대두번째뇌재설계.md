...
date: 2026-07-30
category: 시스템방법론
subject: Tiago Forte — PKM에서 PCM으로, AI 시대 두 번째 뇌 재설계
tags: [AI탐구, deepdive]
starred: false
micro_action: 옵시디언(또는 비서앱)에 "나의 컨텍스트 파일" 신규 노트 하나 생성 — 와당탕 핵심 프로젝트 현황·운영 원칙·올해 목표 3가지를 200자 이내로 정리해 저장
---

# Tiago Forte — PKM에서 PCM으로, AI 시대 두 번째 뇌 재설계

## 누구/무엇인가

Tiago Forte는 2017년부터 "Building a Second Brain(BASB)" 이라는 개인 지식 관리(PKM) 방법론을 온라인 코호트 강의로 대중화한 교육 창업가입니다. 동명의 책은 2022년 출간 이후 2025년까지 누적 판매 40만 부를 돌파했고, Circle 커뮤니티 회원 1만 3천 명, 유튜브 구독자 37만 3천 명으로 PKM 분야 세계 최대 브랜드를 구축했습니다. 2025년 연간 리뷰 기준 Forte Labs의 매출은 $215만(약 29억 원), 순이익은 $65만(약 8.7억 원), 영업이익률 30%로 '수익성의 해'를 선언했습니다. 가장 주목할 변화는 2022년 말 6,000명 이상의 수강생을 가르치던 BASB 코호트를 전면 중단한 것입니다. "AI를 깊이 파고들기 위해 3년간 가르치는 일을 멈췄습니다. 처음부터 다시 설계했습니다(I paused teaching for three years to go deep on AI, rethinking my entire approach from first principles)." — 2026년 AI 두 번째 뇌 프로그램 출시 선언문에서. 그리고 2026년 4월, 완전히 재설계된 방법론 **'The AI Second Brain'** 코호트로 귀환했습니다.
...
**오늘 30분 (micro)**: 옵시디언 또는 비서앱에 `_context/와당탕-컨텍스트.md` 파일 하나 생성. 다음 세 가지만 200자 이내로 각각 정리 → Claude에게 매번 붙여넣지 않고 PROJECT 노트 맨 위에 include해두기. ① 지금 진행 중인 핵심 프로젝트 3개 ② 와당탕연구소·느린호밀 운영 원칙 5가지 ③ 올해 숫자 목표. 이것이 PCM의 첫 번째 실천입니다.

**이번 주 1-2시간 (mid)**: 비서앱 ERP의 현재 옵시디언 구조를 PARA 기준으로 재점검합니다. 폴더 구조가 '주제별' 분류라면 '현재 연관성 기준' 분류로 개편합니다. Projects에는 현재 활성 프로젝트만, Areas에는 지속 관리 영역(재무·운영·콘텐츠)만. Claude Code로 비서앱에 "현재 프로젝트 컨텍스트 자동 주입" 기능 프로토타입을 추가합니다 — 매출 대시보드 조회 시 해당 프로젝트 컨텍스트 파일을 자동으로 시스템 프롬프트에 포함시키는 구조.

```python
# 비서앱 컨텍스트 주입 스니펫 예시
def load_project_context(project_name: str) -> str:
    context_path = f"vault/_context/{project_name}.md"
...
# Claude 호출 시 자동 포함
system_prompt = BASE_PROMPT + "\n\n## 현재 프로젝트 컨텍스트\n" + load_project_context("와당탕")
```

**이번 달 실험 (macro)**: "와당탕 AI 두 번째 뇌" 설계. 와당탕연구소의 4개 핵심 영역(공동구매 운영, 느린호밀 카페, 콘텐츠/강의, 비서앱 개발)별로 컨텍스트 파일 작성 → 비서앱에 Areas별 Claude 에이전트 라우팅 추가. 측정 지표: Claude 답변 품질 점수(5점 척도)를 컨텍스트 있을 때 vs 없을 때 각각 10회 측정해 비교. Forte의 PCM 개념이 실제로 답변 품질을 얼마나 높이는지 수치로 검증합니다.

## 한국 솔로 운영자 맥락에서 주의

**한국형 PARA는 '앱 과의존' 함정이 있습니다.** Forte의 시스템은 노션·옵시디언 같은 서구권 PKM 앱을 전제로 설계됐지만, 한국 솔로 운영자는 카카오워크·네이버 워크플레이스·밴드 등 한국형 협업 도구와 혼용하는 경우가 많습니다. PCM의 핵심인 "AI에게 컨텍스트 주기"가 한국어 문서·카카오 채팅·한글 엑셀 파일에서 제대로 작동하는지 검증 없이 Forte 방법론을 그대로 적용하면 구조만 복잡해지고 실효성이 떨어집니다. 와당탕처럼 실제 운영 데이터(주문·재고·고객 DM)가 카카오채널·네이버 스마트스토어에 분산된 경우, PCM의 첫 단계는 그 데이터를 단일 컨텍스트로 통합하는 '수집 레이어' 구축입니다.

**3년 투자 없이 방법론만 복사하면 겉핥기가 됩니다.** Forte가 3년간 가르치기를 멈추고 AI를 직접 파고든 것이 AI 두 번째 뇌의 핵심입니다. 방법론 교재만 구입하고 실제 AI 업무 통합 없이 적용하면, 노트 앱을 하나 더 추가하는 것과 다르지 않습니다. 한국 솔로 운영자에게 필요한 것은 "내 실제 업무 한 가지"에서 PCM을 30일 실험해보는 것입니다.
