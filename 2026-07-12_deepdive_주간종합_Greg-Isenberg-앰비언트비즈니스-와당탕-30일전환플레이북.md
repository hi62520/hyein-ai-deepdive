...
Isenberg는 2026년 중반 이렇게 썼습니다. *"'ambient businesses'가 온다. 에이전트가 시장을 모니터링하고 고객을 처리하고 결정을 실행한다. 당신은 며칠에 한 번 체크인한다. 7-8자리 매출의 사업이 거의 매일 인간 개입 없이 돌아간다."* 같은 스레드에서 그는 예언했습니다: *"누군가 직원 0명, 외주 0명, 자신과 에이전트 군단만으로 18개월 안에 $100M 사업을 만들 것이다."* 그러나 이 비전을 실제로 구현하려면 하나의 자동화가 아니라 세 개의 레이어가 맞물려야 합니다.
...
그가 직접 답했습니다: *"이 새로운 세계에서 사람은 크리에이티브가 되고, 에이전트는 노동이 되고, 회사 자체가 컨텍스트 레이어가 된다. 그 공유 뇌가 실제 회사다. 사람과 에이전트는 그 뇌에 플러그를 꽂는다."* 그리고: *"에이전트가 80/20 법칙을 폭력적으로 만들었다 — 중요한 20%의 일만 인간이 하고, 80%는 그냥 사라졌다. 수많은 직무 기술서가 그 80% 안에 숨어 있었다."*
...
그는 전환의 핵심 조건도 명확히 제시했습니다: *"2026년에 만들 수 있는 가장 가치 있는 것은 에이전트가 운영할 수 있을 만큼 잘 문서화된 사업이다."* 비서앱·ERP의 모든 SOP, 가격, 권한, 의사결정 규칙이 `CLAUDE.md`와 하네스 파일로 문서화되는 순간, 그것이 앰비언트 전환의 실질적 시작입니다. 또한 중요한 실전 수치: 에이전트 스택 비용은 월 $300~500인 반면, 동등한 역할을 하는 인력 팀의 급여는 월 $80,000~$120,000입니다.
